...
public class SameTree {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(isSameTreeFaster(p, q));
    }
    ...
    public static boolean isSameTree(TreeNode p, TreeNode q) {
        ...
        //Java caches Integer objects for values between -128 and 127. If you create two Integer objects with a value within
        // this range, they will point to the same object in memory. Thus, == would return true for those values.
    ...
    //Better approach:
    //Avoids the overhead of autoboxing and unboxing by directly comparing primitive int values.
    //Reduces the number of method calls by avoiding Objects.equals().
    //Simplifies the logic, making the code more efficient by performing fewer checks.
    //Avoids creating unnecessary objects, reducing memory usage and garbage collection pressure.
    public static boolean isSameTreeFaster(TreeNode p, TreeNode q) {
        if (p == null && q == null) return true;

        if (p == null || q == null || p.val != q.val) return false;

        return isSameTreeFaster(p.left, q.left) && isSameTreeFaster(p.right, q.right);
    }
}
